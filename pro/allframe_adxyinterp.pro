...
; arrays to X/Y do a sparse grid and performing
; linear interpolation to the full size.                                
;
; INPUTS:
...
;   congrid normally does nx_out/nx_in scalling
;   if /minus_one set it does (nx_out-1)/(nx_in-1)
ixx = CONGRID(xxs,nxs*nstep+1,nys*nstep+1,/interp,/minus_one)
iyy = CONGRID(yys,nxs*nstep+1,nys*nstep+1,/interp,/minus_one)
xx[0:nxs*nstep,0:nys*nstep] = ixx
yy[0:nxs*nstep,0:nys*nstep] = iyy

; Deal with left edge
if nxs*nstep lt nx then begin
  ...
  xx[nx-nstep-1:nx-1,0:nys*nstep] = ixx_rt
  yy[nx-nstep-1:nx-1,0:nys*nstep] = iyy_rt
endif
; Deal with right edge
if nys*nstep lt ny then begin
  ...
  xx[0:nxs*nstep,ny-nstep-1:ny-1] = ixx_tp
  yy[0:nxs*nstep,ny-nstep-1:ny-1] = iyy_tp
endif
; Deal with top/right corner
if nxs*nstep lt nx and nys*nstep lt ny then begin
