...
FUNCTION POLY_FITQ, x, y, ndegree, $
    ...
    IF NOT ARG_PRESENT(status) THEN BEGIN
        CASE status OF
        1: if not keyword_set(quiet) then MESSAGE, "Singular matrix detected."
        2: if not keyword_set(quiet) then MESSAGE,/INFO, "Warning: Invert detected a small pivot element."
        ELSE:
        ENDCASE
    ENDIF
