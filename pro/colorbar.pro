...
IF KEYWORD_SET(vertical) THEN BEGIN
   if not keyword_set(bar) then bar = REPLICATE(1B,20) # BINDGEN(ncolors)
   ...
   ENDELSE
ENDIF ELSE BEGIN
   if not keyword_set(bar) then bar = BINDGEN(ncolors) # REPLICATE(1B, 20)
...
IF scalablePixels THEN BEGIN
   ...
   if barsz(0) eq 2 then $
     TV, bar, xstart, ystart, XSIZE=xsize, YSIZE=ysize, /Normal
   if barsz(0) eq 3 then $
     TV, bar, xstart, ystart, XSIZE=xsize, YSIZE=ysize, /Normal,true=1
...
ENDIF ELSE BEGIN
   ...
   if barsz(0) eq 3 then $
     bar = CONGRID(bar, 3, CEIL(xsize*!D.X_VSize), CEIL(ysize*!D.Y_VSize), /INTERP)
   ...
   if barsz(0) eq 3 then $
     TV, bar, xstart, ystart, /Normal,true=1
