function read_resource,file,meta=meta,header=header,nowrite=nowrite

;; Read the file from using the resource file
...
;; If the file exists and is non-zero then just return it
;;=======================================================
info = file_info(file)
...
FITS_READ,tfluxfile,fim,fhead
FITS_READ,tmaskfile,mim,mhead
...
if not keyword_set(nowrite) then $
  MWRFITS,newim,file,newhead,/create
meta = newhead
