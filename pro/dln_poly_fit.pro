; $Id: //depot/Release/ENVI53_IDL85/idl/idldir/lib/poly_fit.pro#1 $
;
;+
; NAME:
;   DLN_POLY_FIT
...
;
; CALLING SEQUENCE:
;   Result = DLN_POLY_FIT(X, Y, Degree)
;
; INPUTS:
...
;   CT, VIS, Dec 2007: If n==m then avoid divide by zero, return sigma=0.
;   CT, VIS, Dec 2011: Per George Lawrence, for YBAND, if MEASURE_ERRORS is
;    omitted, then multiply YBAND by the sqrt(variance). Otherwise, if
;    MEASURE_ERRORS is present, do not multiply.
;   D. Nidever, July 2016: Added capability to measure SIGMA with a
;                            bootstrap analysis.
;-

FUNCTION DLN_POLY_FIT, x, y, ndegree, $
    ...
    if (haveYband) then begin
        ...
        ; If no MEASURE_ERRORS, multiply the error estimates by the
        ; sqrt(variance).
        if (~haveMeasureError) then $
          yband *= var
...
; fill in obsolete arguments, if necessary
    ...
    if keyword_set(bootstrap) and n gt 2 then begin
      ...
      endif else numbootstrap=50L
      ; Bootstrap loop
      btcoef = dblarr(numbootstrap,ndegree+1)
      btstatus = lonarr(numbootstrap)
      ; Initialize seed using X so reproducible
      seed = PICKRNDSEED(x[0])
      For i=0,numbootstrap-1 do begin
        rndind = round( randomu(seed,n)*(n-1) )
        x2 = x[rndind]
        y2 = y[rndind]
        if (haveMeasureError) then begin
          measure_errors2 = measure_errors[rndind]
          coef1 = poly_fit(x2,y2,ndegree,measure_error=measure_errors2,status=status1)
        endif else begin
          coef1 = poly_fit(x2,y2,ndegree,status=status1)
        endelse
        btcoef[i,*] = coef1
        btstatus[i] = status1
      Endfor
      ...
      For i=0,ndegree do begin
        gdboot = where(btstatus eq 0,ngdboot)
