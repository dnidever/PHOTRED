...
;
; INPUTS:
;  tilefile  The tiling file, e.g. "F2.tiles"
...
;
; USAGE:
;  IDL>photred_loadtilefile,'F2.tiles',tilestr
;
...
for i=0,ntilelines-1 do begin
   ...
   if stregex(tilelines[i],'=',/boolean) eq 1 then begin
    dum = strsplit(tilelines,'=',/extract)
    name = dum[0]
    value = dum[2]
...
tilestr = CREATE_STRUCT(projstr,'tiles',tstr,'header',tilehead)
