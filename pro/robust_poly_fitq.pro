FUNCTION ROBUST_POLY_FITQ,X,Y,NDEG,YFIT,SIG, NUMIT=THIS_MANY, DOUBLE=DOUBLE, silent=silent
;+
; NAME:
;	ROBUST_POLY_FIT
;
...
;  Settle for least-squares:
   LSQFIT = 1
   CC = POLY_FITQ( U, V, NDEG, YFIT , DOUBLE=DOUBLE, /quiet)
ENDIF ELSE BEGIN
...
;  Put the leftover points in the middle segment:
   ...
   ENDFOR
;  Now fit:
   CC = POLY_FITQ( R,S, NDEG, DOUBLE=DOUBLE, /quiet )
   YFIT = POLY(U,CC)  
ENDELSE
...
IF NGOOD LT MINPTS THEN BEGIN
   ...
   ENDIF
   IF NGOOD LT MINPTS THEN BEGIN
      if not keyword_set(silent) then PRINT,'ROBUST_POLY_FIT: No Fit Possible!'
      RETURN,0.
   ENDIF
ENDIF
...
; rather than measure errors
  ...
  CC = POLY_FITQ( U, V, NDEG, YFIT, MEASURE_ERRORS = 1/W^2, DOUBLE=DOUBLE, /quiet )
  ISTAT = ROB_CHECKFIT(V,YFIT,EPS,DEL,  SIG,FRACDEV,NGOOD,W,S)
  IF ISTAT EQ 0 THEN GOTO,AFTERFIT
  IF NGOOD LT MINPTS THEN BEGIN
     if not keyword_set(silent) then PRINT,'ROBUST_POLY_FIT: Questionable Fit!'
     BADFIT=1
     GOTO,AFTERFIT
  ENDIF
